...
class CottonFeaturePage extends StatefulWidget {
  final VoidCallback onBack;

  const CottonFeaturePage({
    super.key,
    required this.onBack,
  });

  @override
  State<CottonFeaturePage> createState() => _CottonFeaturePageState();
}

class _CottonFeaturePageState extends State<CottonFeaturePage> {
  ...
  final List<String> soilTypes = ["Black Soil", "Red soil", "Alluvial soil"];
  ...
  void handlePrediction() async {
    if (_formKey.currentState!.validate()) {
      ...
      Map<String, dynamic> cottonPayload = {
        'crop_type': 'cotton',
        'soil_type': soilType,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Cotton Seed Prediction"),
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              _numericInput(label: "Soil pH Level", placeholder: "e.g., 7.2", onChanged: (v) => setState(() => soilPh = v)),
              _numericInput(label: "Temperature (°C)", placeholder: "e.g., 34.5", onChanged: (v) => setState(() => temperature = v)),
              _numericInput(label: "Nitrogen (N) value", placeholder: "e.g., 90.0", onChanged: (v) => setState(() => nitrogenN = v)),
              _numericInput(label: "Phosphorus (P) value", placeholder: "e.g., 35.0", onChanged: (v) => setState(() => phosphorusP = v)),
              _numericInput(label: "Potassium (K) value", placeholder: "e.g., 50.0", onChanged: (v) => setState(() => potassiumK = v)),
